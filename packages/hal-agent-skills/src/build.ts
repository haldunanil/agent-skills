...
async function build() {
  try {
    ...
    if (registered.length === 0) {
      console.log('No rule-based skills registered. Skipping AGENTS.md generation.')
      return
    }
    ...
    if (buildAll) {
      ...
      console.log('No --skill=<name> or --all flag provided. Pass --all to build every registered skill, or --skill=<name> to build one.')
      return
    }
